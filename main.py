...
Eylem: <code>{kwargs.get('eylem','')}</code>
    ...
    """, disable_web_page_preview=True)
        ...
        #    bot.send_message(kurucu_id, str(e))
    try:
        await bot.set_chat_title(-1001943983208, f"Best Game Log - {len(oyunlar)}")
    except Exception as e:
        if "chat not found" in str(e):
            pass
...
async def start_private(message): #, **kwargs
    ...
    if chat_tipi == "private":
        ...
        if len(ayrik) == 2:
            ...
            callback_button = types.InlineKeyboardButton(text="Botu grubuna ekle 👪", url="https://t.me/Sohbet_Sarayıt?startgroup=a")
            callback_button2 = types.InlineKeyboardButton(text="Destek ve İletişim ⚙️", url="https://t.me/Sohbet_Sarayı")
...
async def baslat(message):
    ...
    await bot.send_message(chat_id, f"🐙 <b>Best Game Bot</b>'a hoş geldiniz! Lütfen bir oyun tipi seçiniz.", reply_markup=keyboard)
...
async def callback_inline(cagri): #çağrıcı cagrici
    ...
    # or " " + first_name +" kelime" in cagri.message.text
        if acan_id == user_id:
            ...
            #if sorgu == "kelime_gir":
            #    bot.answer_callback_query(cagri.id, url = "t.me/OctopusGame_Bot?start=test")
                #try:
                ...
                #except:
                #    bot.answer_callback_query(cagri.id, url = "telegram.me/OctopusGame_Bot?start=start")
...
🙏 Yardım ve sorularınız için: @bestmusicdestek""")
